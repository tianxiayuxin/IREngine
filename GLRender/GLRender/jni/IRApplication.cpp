...
void IRApplication::start()
{
	pthread_create(&_threadId, 0, threadStartCallback, this);
	return;
}

void IRApplication::stop()
{
...
void IRApplication::setWindow(ANativeWindow *window)
{
	pthread_mutex_lock(&_mutex);
	_msg = MSG_SET_WINDOW;
	_window = window;
...
bool IRApplication::initialize(ConfigFlag flags)
{
...
void IRApplication::drawFrame()
{
	glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);
}

EGLint IRApplication::getContextRenderableType(EGLDisplay eglDisplay)
{
...
void *IRApplication::threadStartCallback(void *it)
{
	IRApplication *ir = (IRApplication*)it;
	ir->renderLoop();
	pthread_exit(0);
	return 0;
}
